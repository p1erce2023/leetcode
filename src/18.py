class Solution:
    ...
    def fourSum(self, nums, target):
        # many pruning techniques can be done. 140ms, top, amazing fast!
        if len(nums) < 4:
            return []

        nums.sort()
        if not sum(nums[:4]) <= target <= sum(nums[-1:-5:-1]):
            return []
        ...
        for i in xrange(n-3):
            ...
            if sum(nums[i:i+4]) > target:  # too big
                break

            if nums[i] + nums[-3] + nums[-1] + nums[-2] < target:  # too small
                continue
            ...
            for j in xrange(i+1, n-2):
                ...
                if nums[i] + nums[j] + nums[-1] + nums[-2] < target:  # too small
                    continue
                ...
                while k < l:
                    ...
                    if s == target:
                        ...
                        k += 1
                        l -= 1
                        while nums[k] == nums[k-1] and k < l:  # avoid duplicate
                            k += 1
                        while nums[l] == nums[l+1] and k < l:  # avoid duplicate
                            l -= 1
                    elif s > target:
                        l -= 1
                        while nums[l] == nums[l+1] and k < l:  # avoid duplicate
